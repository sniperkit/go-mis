...
func GetAllAgentByBranchID(ctx *iris.Context) {
	...
	query := "SELECT agent.\"picUrl\", agent.\"username\", agent.fullname, agent.address,  "
	...
	query += "(SELECT \"name\" FROM inf_location WHERE province = agent.province AND city = agent.city AND kecamatan = agent.kecamatan AND kelurahan = agent.kelurahan LIMIT 1) AS \"kecamatan\" "
	query += "FROM agent "
	query += "INNER JOIN r_branch_agent ON r_branch_agent.\"agentId\" = agent.id "
	query += "WHERE r_branch_agent.\"branchId\" = ? AND agent.\"deletedAt\" IS NULL"
